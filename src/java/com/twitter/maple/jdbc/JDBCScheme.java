...
public class JDBCScheme extends Scheme<JobConf, RecordReader, OutputCollector, Object[], Object[]>
{
    ...
    public void sink( FlowProcess<JobConf> flowProcess, SinkCall<Object[], OutputCollector> sinkCall ) throws IOException {
        ...
        if( updateBy != null )
        {
            TupleEntry allValues = tupleEntry.selectEntry( updateValueFields );
            TupleEntry updateValues = tupleEntry.selectEntry( updateByFields );

            allValues = cleanTuple( allValues );

            if( updateValues.equals( updateIfTuple ) )
                outputCollector.collect( null, allValues );
            else
                outputCollector.collect( updateValues, allValues );

            return;
        }

        TupleEntry result = tupleEntry.selectEntry( getSinkFields() );
     ...
     * @return
     */
    protected TupleEntry cleanTuple( TupleEntry result ) {
        return result;
    }
